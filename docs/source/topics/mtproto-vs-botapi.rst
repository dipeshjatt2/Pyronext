...
Pyronext is a framework written from the ground up that acts as a fully-fledged Telegram client based on the MTProto
API. This means that Pyronext is able to execute any official client and bot API action and more. This page will
therefore show you why Pyronext might be a better choice for your project by comparing the two APIs, but first, let's
...
Pyronext, implement all these details.
...
Here is a non-exhaustive list of all the advantages in using MTProto-based libraries -- such as Pyronext -- instead of
the official HTTP Bot API. Using Pyronext you can:
