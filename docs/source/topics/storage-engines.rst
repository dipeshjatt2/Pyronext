...
Every time you login to Telegram, some personal piece of data are created and held by both parties (the client, Pyronext
...
authorization process from scratch each time, Pyronext needs to store the generated session data somewhere.

Different Storage Engines
-------------------------

Pyronext offers two different types of storage engines: a **File Storage** and a **Memory Storage**.
...
``my_account.session``. Any subsequent client restart will make Pyronext search for a file named that way and the
...
restart will make Pyronext search for a database named that way and the session database will be automatically loaded.
...
Session strings are useful when you want to run authorized Pyronext clients on platforms where their ephemeral
